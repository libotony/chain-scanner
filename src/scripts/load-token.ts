...
const makeFileContent = (list: { [index: string]: { [index: string]: Token } }, main: string[], test: string[]) => {
    let asset = `export enum AssetType { VET = 0, VTHO, '${main.join("', '")}'`
    if (test.length) { 
        asset += `, '${test[0]}' = 100`
        if (test.length > 1) {
    ...
    const AssetConfig: {[index:string]:{type:number, decimals: number}} = {
        VET: {
            type: 0,
            decimals: 18
        },
    ...
    for (let i = 0; i < main.length; i++) {
        AssetConfig[main[i]] = {
            type: i + 2,
            decimals: list[Network.MainNet][main[i]].decimals
        }
    }
    ...
    for (let i = 0; i < test.length; i++) {
        AssetConfig[test[i]] = {
            type: i + 100,
            decimals: list[Network.TestNet][test[i]].decimals
        }
    }
